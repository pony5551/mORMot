...
{$endif FPC}
  ...
  protected
    ///pony add start
    //HttpAPi Request Object and buffer Start
    fReqBuf: SockString;
    fReqBufLen: Cardinal;
...
///pony modfy end
    ...
    procedure DoProcessContext(AContext: THttpServerRequest);
...
type
  TServerWorkItem = class
  private
    FServer: THttpApiServer;
    FContext: THttpServerRequest;
  public
    constructor Create(AContext: THttpServerRequest; AServer: THttpApiServer);
    destructor Destroy; override;
    property Context: THttpServerRequest read FContext;
    property Server: THttpApiServer read FServer;
  end;

{ TServerWorkItem }
constructor TServerWorkItem.Create(AContext: THttpServerRequest; AServer: THttpApiServer);
begin
  FContext := AContext;
  FServer := AServer;
end;

destructor TServerWorkItem.Destroy;
begin
  FContext.Free;
  inherited;
end;
...
procedure THttpApiServer.DoProcessContext(AContext: THttpServerRequest);
type
  ...
  begin
    ...
    if AContext.OutContentType=HTTP_RESP_STATICFILE then begin
      ...
      try // http.sys will serve then close the file from kernel
        ...
        Http.SendHttpResponse(fReqQueue,
          Req^.RequestId,flags,Resp^,nil,bytesSent,nil,0,nil,{fLogData}nil);
      finally
...
begin
  ...
  if fLoggingServiceName<>'' then
  begin
    CurrentLog^.ServiceNameLength := Length(fLoggingServiceName);
    CurrentLog^.ServiceName := PAnsiChar(fLoggingServiceName);
  end;
  Verbs := VERB_TEXT;
  ...
  try
    ...
    AContext.fConnectionID := Req^.ConnectionId;
    AContext.fHttpApiRequest := Req;
    AContext.fURL := Req^.pRawUrl;
    if Req^.Verb in [low(Verbs)..high(Verbs)] then
      AContext.fMethod := Verbs[Req^.Verb] else
      SetString(AContext.fMethod,Req^.pUnknownVerb,Req^.UnknownVerbLength);
    with Req^.Headers.KnownHeaders[reqContentType] do
      SetString(AContext.fInContentType,pRawValue,RawValueLength);
    ...
    AContext.fUseSSL := Req^.pSslInfo<>nil;
    AContext.fInHeaders := RetrieveHeaders(Req^,fRemoteIPHeaderUpper,RemoteIP);
    ...
    if HTTP_REQUEST_FLAG_MORE_ENTITY_BODY_EXISTS and Req^.Flags<>0 then begin
      ...
      if InContentLength<>0 then begin
        SetLength(AContext.fInContent,InContentLength);
        BufRead := pointer(AContext.InContent);
        ...
        repeat
          ...
          if Err=ERROR_HANDLE_EOF then begin
            if InContentLengthRead<InContentLength then
              SetLength(AContext.fInContent,InContentLengthRead);
            Err := NO_ERROR;
...
procedure THttpApiServer.WorkItemCallBack(Sender: TObject);
var
  LContext: THttpServerRequest;
begin
  try
    if TServerWorkItem(Sender).Server.Terminated then Exit;
    LContext := TServerWorkItem(Sender).Context;
    DoProcessContext(LContext);
  finally
    TServerWorkItem(Sender).Free;
  end;
end;
...
begin
  // THttpServerGeneric thread preparation: launch any OnHttpThreadStart event
  NotifyThreadStart(self);
  //Http.ReceiveHttpRequest Loop
  repeat
    LContext := GetContext;
    if (LContext <> nil) and not Terminated then
    begin
      if not FThreadPool.QueueUserWorkItem(WorkItemCallBack, TServerWorkItem.Create(LContext, Self)) then
...
//        'text/html; charset=utf-8');
//      Http.SendHttpResponse(fReqQueue,
//        Req^.RequestId,0,Resp^,nil,bytesSent,nil,0,nil,{fLogData}nil);
//    except
//      on Exception do
//        ; // ignore any HttpApi level errors here (client may crashed)
//    end;
...
//        Resp^,nil,bytesSent,nil,0,nil,{fLogData}nil));
//    end;
//  end;
//
//begin
...
//        // retrieve any SetAuthenticationSchemes() information
//        if byte(Context.fAuthenticationSchemes)<>0 then // set only with HTTP API 2.0
//          for i := 0 to Req^.RequestInfoCount-1 do
//          if Req^.pRequestInfo^[i].InfoType=HttpRequestInfoTypeAuth then
//            with PHTTP_REQUEST_AUTH_INFO(Req^.pRequestInfo^[i].pInfo)^ do
//            case AuthStatus of
...
//    THttpApiServer(fClones.List{$ifdef FPC}^{$endif}[i]).fLogData := nil;
  if (self=nil) or (fClones=nil) then
    exit;
  fLoggined := False;
  for i := 0 to fClones.Count-1 do
    THttpApiServer(fClones.List{$ifdef FPC}^{$endif}[i]).fLoggined := fLoggined;
end;
